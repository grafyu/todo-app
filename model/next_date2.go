...
func NextDate2(now time.Time, date string, repeat string) (string, error) {
	...
	case "w":
		...
		return nextCheckDay(next, daysNumb, weekdays)
...
func nextCheckDay(next time.Time, daysNumb []string, weekdays map[string]stirng) time.Time {
	var days []string

	for dn := range daysNumb {
		days = append(days, weekdays[dn])
	}

	checkDayNow := slices.Contains(days, next.Weekday())

	// если данный "next" это checkDayNow, то пропускаем его поиск
	if !checkDayNow {
		for !slices.Contains(days, next.Weekday()) {
			next = next.AddDate(0, 0, 1)
		}
	}
